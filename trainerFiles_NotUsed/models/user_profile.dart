class UserProfile {
  ...
  factory UserProfile.fromJson(Map<String, dynamic> json) {
    return UserProfile(
      id: json['id'] ?? json['userId'], // Backend uses 'userId' field
      ...
      zip: json['zip'] ?? '',
      gender: json['gender'] ?? '',
      // Handle both formats: direct profileImage or images array - store keys, not URLs
      profileImage:
          json['profileImage'] ??
          (json['images'] != null && (json['images'] as List).isNotEmpty
              ? (json['images'] as List).first['imageId']
              : null),
      // Backend stores idImageKey as a string, but might also have idImage as a Map - store key, not URL
      idImage:
          json['idImageKey'] ??
  ...
  bool get isProfileComplete {
    return firstName.isNotEmpty &&
        ...
        bio.isNotEmpty &&
        phone.isNotEmpty &&
        // specialty is now optional, so we don't check it
        address1.isNotEmpty &&
        city.isNotEmpty &&
        state.isNotEmpty &&
        zip.isNotEmpty &&
